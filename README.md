...
- Review-only operator playbooks (`GET /api/v1/playbooks`), threat-intel list preview that applies nothing (`POST /api/v1/intel/preview`), destination-flow SIEM export (`GET /api/v1/export/flows`), and an actor/action/hour audit rollup (`GET /api/v1/audit/summary`), plus an optional best-effort syslog push sink (`NETRA_SYSLOG_ADDR`, off by default, leader-only in HA) alongside the pull-based SIEM export above. Dashboard **Report** page. See `docs/siem-export.md`.
...
```bash
docker build -t ghcr.io/zyvorai/netra:0.27.58 .
docker build -f Dockerfile.agent -t ghcr.io/zyvorai/netra-agent:0.27.58 .
```
