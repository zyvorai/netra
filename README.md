...
- Review-only operator playbooks (`GET /api/v1/playbooks`), threat-intel list preview that applies nothing (`POST /api/v1/intel/preview`), destination-flow SIEM export (`GET /api/v1/export/flows`), an actor/action/hour audit rollup (`GET /api/v1/audit/summary`), and a per-node hook/program coverage matrix (`GET /api/v1/ebpf/coverage`, `netractl ebpf coverage`) — attached vs detached programs, missing maps, stale agents — plus optional best-effort push sinks — syslog (`NETRA_SYSLOG_ADDR`) and Snowflake (`NETRA_SNOWFLAKE_ACCOUNT`, audit events only) — both off by default and leader-only in HA, alongside the pull-based SIEM export above. Dashboard **Report** page. See `docs/siem-export.md`.
