...
(async () => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/v1/status') body = { fastPath: { mode: 'observe', shield: { mode: 'audit' } } };
  ...
  await page.getByText('1 agent reports are stale.', { exact: false }).waitFor();
  ...
  await page.getByText('No agents have reported.', { exact: false }).waitFor();
  ...
  await browser.close();
  console.log('PASS: navigation, scoped reload, evidence drawer/focus, workload drill-down, history, pause, failure retention, empty coverage, mobile, dark theme; no page errors.');
