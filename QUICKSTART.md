...
SSH host gets K3s (or uses existing), Cilium with Hubble Relay, then Netra via Helm. Image tag is reused (`0.22.0`); after deploy, restart the Deployment so the new layers are picked up:
