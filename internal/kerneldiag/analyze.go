...
func Build(agents []models.AgentStatus) models.KernelNetworkDiagnosticsResponse {
	out := models.KernelNetworkDiagnosticsResponse{
		Limitations: []string{
			"Kernel and interface counters are cumulative since node boot; compare two reports before treating them as a rate.",
...
func analyzeNode(a models.AgentStatus) []models.KernelNetworkFinding {
	...
	if a.Stack.SoftnetDropped > 0 {
		...
		add(models.KernelNetworkFinding{
			Severity: severity(a.Stack.SoftnetDropped, 1000), Layer: "softnet-backlog", Signal: "ingress backlog overflow",
	...
	if a.Stack.SoftnetTimeSqueeze > 0 {
		...
		add(models.KernelNetworkFinding{
			Severity: severity(a.Stack.SoftnetTimeSqueeze, 1000), Layer: "softnet-budget", Signal: "network softirq budget exhaustion",
	...
	if v := counters["TcpExt.TCPBacklogDrop"] + counters["TcpExt.TCPRcvQDrop"] + counters["TcpExt.TCPZeroWindowDrop"]; v > 0 {
		add(models.KernelNetworkFinding{
			Severity: severity(v, 1000), Layer: "tcp-receive", Signal: "TCP socket receive/backlog pressure",
	...
	if v := counters["Udp.RcvbufErrors"] + counters["Udp.MemErrors"]; v > 0 {
		...
		add(models.KernelNetworkFinding{
			Severity: severity(v, 1000), Layer: "socket-receive", Signal: "UDP receive buffer exhaustion",
	...
	if v := counters["Udp.SndbufErrors"]; v > 0 {
		...
		add(models.KernelNetworkFinding{
			Severity: severity(v, 1000), Layer: "socket-send", Signal: "UDP send buffer exhaustion",
	...
	if listen > 0 {
		...
		add(models.KernelNetworkFinding{
			Severity: severity(listen, 1000), Layer: "tcp-listen", Signal: "TCP accept/SYN queue overflow",
	...
	if memory > 0 {
		add(models.KernelNetworkFinding{
			Severity: severity(memory, 100), Layer: "tcp-memory", Signal: "TCP memory pressure",
	...
	if qdiscDrops > 0 {
		add(models.KernelNetworkFinding{
			Severity: severity(qdiscDrops, 1000), Layer: "qdisc", Signal: "egress queue drops", Evidence: []string{fmt.Sprintf("qdisc_drops=%d", qdiscDrops)},
	...
	if rxMissed > 0 || ifaceDrops > 0 {
		add(models.KernelNetworkFinding{
			Severity: severity(rxMissed+ifaceDrops, 1000), Layer: "nic-driver", Signal: "interface receive/transmit loss",
	...
	if v := counters["Ip.InDiscards"] + counters["Ip.OutDiscards"] + counters["IpExt.InNoRoutes"] + counters["IpExt.OutNoRoutes"]; v > 0 {
		add(models.KernelNetworkFinding{
			Severity: severity(v, 1000), Layer: "ip", Signal: "IP-layer discard or route failure",
