...
**Still not yet done**: a live-cluster HA-failover check — force a leader failover (`docs/high-availability.md`) and confirm exactly one replica's log shows reconciliation activity (`"gitops applied"`/`"gitops drift detected"`) at any given time. Double-reconciliation across replicas racing to apply the same manifest is exactly the failure mode `gate.Promote`/`Demote` exists to prevent, but it only shows up under a real failover — unit tests alone can't establish it, the same way this project's kernel/BPF features require a live-kernel check beyond `go test`.

A first attempt at this check (2026-09-14, against a live single-node k3s cluster) found and fixed a real, unrelated bug instead of completing the failover test: `internal/kube.TryAcquireOrRenewLease`/`ReleaseLease` formatted Lease timestamps with a layout a real Kubernetes API server's `metav1.MicroTime` decoding rejects outright (see the 0.27.47 CHANGELOG entry) — HA leader election had never actually worked against a real cluster, only against the fake HTTP backend `internal/kube`'s own unit tests use. That's fixed and covered by a new regression test. The failover check itself is still outstanding: the same attempt also hit an unrelated live-cluster rollout deadlock (the two now-obsolete pre-fix HA pods never got cleaned up by the deployment controller after the fix was redeployed, for reasons that look environmental to that specific shared host rather than anything in this chart or `internal/gitops`/`internal/kube`) and had to stop there.
