...
`netra_l7_tls_sni`/`netra_l7_http_host` are `noinline` (unlike almost everything else in this file) purely to isolate their own verifier budget from the CT/policy/DNS logic inlined ahead of them — a real, if partial, improvement (roughly 19% fewer processed instructions), but it does not clear the fixed threshold above on affected kernels. The only credible further fix identified but not attempted is a bulk-copy-then-scan rewrite (copy the relevant bytes into a local scratch buffer before parsing, avoiding packet-pointer arithmetic inside the loop) — a materially more invasive change than anything above. Until then, `NETRA_L7=auto`'s graceful degradation is the practical behavior on affected kernels.
