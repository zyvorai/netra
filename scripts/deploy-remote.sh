...
build_image() {
  ...
  if command -v go >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
    ...
    \$runtime build -f Dockerfile.runtime -t ghcr.io/zyvorai/netra:0.13.0 .
    return 0
  fi
  if [[ -z "\$runtime" ]]; then
    echo "podman or working docker required to build controller image" >&2
  ...
  \$runtime build -t ghcr.io/zyvorai/netra:0.13.0 .
}

import_image() {
  if command -v podman >/dev/null 2>&1; then
    podman save ghcr.io/zyvorai/netra:0.13.0 | sudo k3s ctr images import -
  else
    docker save ghcr.io/zyvorai/netra:0.13.0 | sudo k3s ctr images import -
  fi
}
