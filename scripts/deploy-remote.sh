...
build_image() {
  ...
  if command -v go >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
    ...
    \$runtime build -f Dockerfile.runtime -t ghcr.io/zyvorai/netra:0.27.54 .
    return 0
  fi
  if [[ -z "\$runtime" ]]; then
    echo "podman or working docker required to build controller image" >&2
  ...
  \$runtime build -t ghcr.io/zyvorai/netra:0.27.54 .
}

import_image() {
  if command -v podman >/dev/null 2>&1; then
    podman save ghcr.io/zyvorai/netra:0.27.54 | sudo k3s ctr images import -
  else
    docker save ghcr.io/zyvorai/netra:0.27.54 | sudo k3s ctr images import -
  fi
}
...
build_agent_image() {
  ...
  \$runtime build -f Dockerfile.agent -t ghcr.io/zyvorai/netra-agent:0.27.54 .
}

import_agent_image() {
  if command -v podman >/dev/null 2>&1; then
    podman save ghcr.io/zyvorai/netra-agent:0.27.54 | sudo k3s ctr images import -
  else
    docker save ghcr.io/zyvorai/netra-agent:0.27.54 | sudo k3s ctr images import -
  fi
}
...
if [[ "\$AGENT_ENABLED" == "true" ]]; then
  AGENT_SET=(--set agent.enabled=true --set agentImage.repository=ghcr.io/zyvorai/netra-agent --set agentImage.tag=0.27.54)
fi
...
# image.tag is a fixed value ("0.27.54"), not a per-build digest/tag, so the
